...
{
    public partial class AdmResultadoUltimaLicitaciónLEBACS : WebFormBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.MenuTab2.CurrentMenuItem = "AdmLicitacionesLEBACS";
        ...
        public void CargarGrilla()
        {
            try
            {
                var calendario = ServicioSistema<CalendarioLicitacionesLEBACS>.GetAll().ToList().OrderBy(x => x.Fecha);
                
                dgvCalendario.DataSource = calendario;
                
                dgvCalendario.DataBind();

                var resultado = ServicioSistema<ResultadoUltimaLicitacionLEBACS>.GetAll().ToList().OrderByDescending(y => y.EsUltimaLicitacion).ThenBy(x => x.Plazo);
        ...
        protected void btnNuevoCalendario_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                ServicioSistema<CalendarioLicitacionesLEBACS>.SaveOrUpdate(new CalendarioLicitacionesLEBACS { Fecha = Convert.ToDateTime(fecha.Value), HorarioLicitacionHB = horario.Value });
                var item = ServicioSistema<CalendarioLicitacionesLEBACS>.GetAll()
        ...
        protected void btnNuevoResultado_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                ServicioSistema<ResultadoUltimaLicitacionLEBACS>.SaveOrUpdate(new ResultadoUltimaLicitacionLEBACS { Plazo = Convert.ToInt32(plazo.Value), TasaCorte = Convert.ToDecimal(tasaDeCorte.Value), EsUltimaLicitacion = EsUltimaLicitacion.Checked });
                var items = ServicioSistema<ResultadoUltimaLicitacionLEBACS>.GetAll()
        ...
        {
            return ServicioSistema<CalendarioLicitacionesLEBACS>.Get(calendario => calendario.Fecha == fecha).Count() > 0;
        }
        ...
        protected void btnBorrarCalendario_Click(object sender, EventArgs e)
        {
            ...
            ServicioSistema<CalendarioLicitacionesLEBACS>.Delete(new CalendarioLicitacionesLEBACS { IdCalendarioLicitacionesLEBACS = Convert.ToInt32(gvr.Cells[0].Text) });
            var items = ServicioSistema<CalendarioLicitacionesLEBACS>.GetAll()
                          .ToList().OrderBy(x => x.Fecha);
        ...
        protected void btnBorrarResultado_Click(object sender, EventArgs e)
        {
            ...
            ServicioSistema<ResultadoUltimaLicitacionLEBACS>.Delete(new ResultadoUltimaLicitacionLEBACS { IdResultadoUltimaLicitacionLEBACS = Convert.ToInt32(gvr.Cells[0].Text) });
            var items = ServicioSistema<ResultadoUltimaLicitacionLEBACS>.GetAll()
                          .ToList().OrderByDescending(y => y.EsUltimaLicitacion).ThenBy(x => x.Plazo);
